...
const WAITAGENT_PANE_TARGET_ID_OPTION: &str = "@waitagent_target_id";
...
impl MainSlotRuntime {
    ...
    pub fn ensure_initial_target_materialized(
        ...
        if let Some(existing_target) = sessions.iter().find(|session| session.is_target_host()) {
            ...
            self.configure_main_pane_output_bridge_for_active_target(
                workspace,
                Some(existing_target.address.qualified_target().as_str()),
            )?;
            ERROR_LOG.log(format!(
                "[diag-newhost] materialize existing_bridge socket={} session={} target={} elapsed={:?} total={:?}",
                workspace.socket_name.as_str(),
                workspace.session_name.as_str(),
                existing_target.address.qualified_target(),
                t_bridge.elapsed(),
        ...
        ERROR_LOG.log(format!(
            "[diag-newhost] materialize new_bridge socket={} session={} target={} elapsed={:?} total={:?}",
            workspace.socket_name.as_str(),
            workspace.session_name.as_str(),
            target.address.qualified_target(),
            t_bridge.elapsed(),
    ...
    pub fn run_main_pane_died(&self, command: MainPaneDiedCommand) -> Result<(), LifecycleError> {
        ...
        self.reconcile_main_pane_exit(&workspace, &current_workspace, &command)
    }

    fn reconcile_main_pane_exit(
        &self,
        workspace: &TmuxWorkspaceHandle,
        current_workspace: &CurrentWorkspace,
        command: &MainPaneDiedCommand,
    ) -> Result<(), LifecycleError> {
        ...
        if let Some(event_generation) = command.pane_generation.as_deref() {
            if current_generation != event_generation {
                let pane_liveness = self.pane_liveness_on_socket(&workspace, &command.pane_id)?;
                if pane_liveness != Some(false) {
                    ERROR_LOG.log(format!(
                        "[diag-bug] run_main_pane_died ignored stale generation: pane={} event_generation={} current_generation={} pane_liveness={pane_liveness:?}",
                        command.pane_id, event_generation, current_generation
                    ));
                    return Ok(());
                }
                ERROR_LOG.log(format!(
                    "[diag-bug] run_main_pane_died accepted stale generation for dead current main pane: pane={} event_generation={} current_generation={}",
                    command.pane_id, event_generation, current_generation
                ));
            }
        } else {
        ...
        match next_target {
            ...
            None => {
                ...
                if result.is_ok() {
                    self.unregister_live_workspace_socket(workspace.socket_name.as_str());
                    if let Some(target_session_name) =
                        Self::active_target_session_name(active_target.as_deref())
                    {
                        self.notify_session_sync_local_target_exited(
    ...
    fn activate_target_in_workspace(
        ...
        if active_target.is_some() && switching_from_remote {
    ...
    fn activate_remote_target_in_workspace(
                ...
                .is_some()
        {
            ...
            self.layout_runtime
                .sync_main_slot_bindings(&workspace, &current_workspace.workspace_dir)?;
            if let Some(session_pane) = self.find_session_pane(&workspace, &qualified_target)? {
        ...
        let session_pane = self.remote_session_pane_for_activation(
            current_workspace,
            &workspace,
            &workspace_main_pane,
            target,
    ...
    fn remote_session_pane_for_activation(
        &self,
        current_workspace: &CurrentWorkspace,
        workspace: &TmuxWorkspaceHandle,
    ...
    fn prepare_existing_remote_session_pane(
        &self,
        workspace: &TmuxWorkspaceHandle,
        pane: &TmuxPaneId,
    ...
    pub fn run_remote_target_exited(
        ...
        let cleanup_pane = session_pane.clone();
        ...
        match transition {
            ...
            RemoteTargetExitTransition::ActivateLocal(target) => {
                ...
                self.activate_target_in_workspace(&current_workspace, &target)?;
                ERROR_LOG.log_exit_latency(format!(
                    "[diag-exit] workspace_remote_exit_activate_local target={} next={} elapsed={:?} total={:?} stage=workspace_exit",
                    command.target,
                    target.address.qualified_target(),
                    t_activate.elapsed(),
                    t_total.elapsed()
    ...
    ) -> Result<RemoteTargetExitTransition, LifecycleError> {
        let sessions = self.workspace_visible_targets(
            workspace.socket_name.as_str(),
            workspace.session_name.as_str(),
            active_target,
    ...
    fn prepare_remote_main_pane_for_local_switch(
        ...
        let parking_pane =
            self.create_workspace_parking_pane(current_workspace, workspace, workspace_main_pane)?;
    ...
    fn restore_workspace_main_pane(
        ...
        let _ = self.select_workspace_content_pane(workspace, pane);
        self.backend
            .set_pane_option(workspace, pane, "remain-on-exit", "on")
            .map_err(main_slot_error)?;
    ...
    fn assign_remote_target_to_content_pane(
        ...
        if self.pane_is_chrome(workspace, pane.as_str())? {
            return Err(LifecycleError::Protocol(format!(
                "refusing to assign remote target `{}` to chrome pane `{}`",
                target.address.qualified_target(),
                pane.as_str()
            )));
        }
    ...
    fn local_target_content_pane(
        ...
        let pane = self.create_or_adopt_local_target_content_pane(workspace, qualified_target)?;
        self.set_local_target_pane_identity(workspace, &pane, target_session_name)?;
        self.set_workspace_main_pane(workspace, &pane)?;
        self.signal_local_target_presentation_refresh(workspace, target_session_name);
        Ok(pane)
    ...
    fn create_or_adopt_local_target_content_pane(
        &self,
        workspace: &TmuxWorkspaceHandle,
        qualified_target: &str,
    ) -> Result<TmuxPaneId, LifecycleError> {
        ...
        self.backend
            ...
            .map_err(|error| {
                LifecycleError::Protocol(format!(
                    "target `{}` has no available content pane: {error}",
                    qualified_target
                ))
            })
    }
    ...
    fn set_local_target_pane_identity(
        ...
        self.set_content_pane_owner(
            ...
            pane,
            target_session_name,
            &format!("{}:{}", workspace.socket_name.as_str(), target_session_name),
        )?;
        Ok(())
    }

    fn set_content_pane_owner(
        &self,
        workspace: &TmuxWorkspaceHandle,
        pane: &TmuxPaneId,
    ...
    fn pane_matches_session_instance(
        ...
        let role = self
            .backend
            .show_pane_option_on_socket(&workspace.socket_name, pane, WAITAGENT_PANE_ROLE_OPTION)
            .map_err(main_slot_error)?
            .unwrap_or_default();
        let owner = self
            .backend
            ...
            .map_err(main_slot_error)?
            .unwrap_or_default();
        Ok(role == WAITAGENT_PANE_ROLE_CONTENT && owner == session_instance_id)
    }
    ...
    fn read_live_session_pane_option_for_target(
        ...
        if self.pane_exists_on_socket(workspace, pane.as_str())?
            && self.pane_is_live_on_socket(workspace, pane.as_str())?
            && self.pane_matches_session_instance(workspace, &pane, session_instance_id)?
        {
            return Ok(Some(pane));
        }
        self.backend
    ...
    fn find_content_pane_by_session_instance(
        &self,
        workspace: &TmuxWorkspaceHandle,
        session_instance_id: &str,
    ) -> Result<Option<TmuxPaneId>, LifecycleError> {
        ...
        match matches.len() {
            ...
            _ => Err(LifecycleError::Protocol(format!(
                "session instance `{session_instance_id}` owns multiple content panes"
            ))),
        }
    }
    ...
    fn set_workspace_main_pane_if_session_exists(
        &self,
        workspace: &TmuxWorkspaceHandle,
        pane: &TmuxPaneId,
    ) -> Result<(), LifecycleError> {
        if !self.workspace_session_exists(workspace)? {
            return Ok(());
        }
        match self
            .backend
    ...
    fn find_session_pane(
            ...
            .filter(|pane| !pane.is_empty())
        {
            ...
            if self.pane_liveness_on_socket(workspace, pane_id.as_str())? == Some(true)
                && self.pane_matches_session_instance(workspace, &pane_id, session_instance_id)?
            {
    ...
    fn select_workspace_content_pane(
        &self,
        workspace: &TmuxWorkspaceHandle,
        pane: &TmuxPaneId,
    ) -> Result<(), LifecycleError> {
        ...
        let pane_session = parts.next().unwrap_or_default();
        let pane_window_index = parts.next().unwrap_or_default();
        let pane_window_id = parts.next().unwrap_or_default();
        let window_target = if !pane_session.is_empty() && !pane_window_index.is_empty() {
            format!("{}:{}", pane_session, pane_window_index)
        } else {
            pane_window_id.to_string()
        };
...
enum RemoteTargetExitTransition {
...
fn target_session_name(target: &str) -> Option<&str> {
    split_qualified_target(target).map(|(_, session_name)| session_name)
}
...
fn next_remote_target_on_active_authority(
    sessions: &[ManagedSessionRecord],
    active_target: Option<&str>,
) -> Option<ManagedSessionRecord> {
    let active_authority = active_target
        .and_then(split_qualified_target)
        .map(|(authority, _)| authority);
    next_remote_target_matching_authority(sessions, active_target, active_authority)
}

fn next_remote_target(
    sessions: &[ManagedSessionRecord],
    active_target: Option<&str>,
) -> Option<ManagedSessionRecord> {
    next_remote_target_matching_authority(sessions, active_target, None)
}

fn next_remote_target_matching_authority(
    sessions: &[ManagedSessionRecord],
    active_target: Option<&str>,
