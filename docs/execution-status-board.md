...
- a dedicated authority transport runtime now exists on the other side of that socket boundary, so a PTY-owning node can connect or register, receive routed `raw_pty_input` plus `apply_resize`, and send `raw_pty_output` back into the interactive raw PTY path; the next gap is binding a real authority-side PTY host onto that runtime instead of relying on transport-only tests
